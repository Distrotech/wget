		      ...
		      wget_buffer_t **ocsp_data, gnutls_datum_t *nonce)
{
	...
	wget_http_add_header_line(req, "Accept-Encoding: identity\r\n");
	wget_http_add_header_line(req, "Accept: */*\r\n");
	wget_http_add_header_line(req, "Content-Type: application/ocsp-request\r\n");
	wget_http_add_header_printf(req, "Content-Length: %u", body.size);
	wget_http_add_header_line(req, "Connection: close\r\n");
...
//void *wget_ssl_open(int sockfd, const char *hostname, int connect_timeout)
