...
#else
						ssize_t nbytes = -1;
						if (lseek(fd, part->position, SEEK_SET) == 0 && (nbytes = write(fd, resp->body->data, resp->body->length)) == (ssize_t)resp->body->length)
#endif
...
//	int max_redirect = 3;
	...
	while (iri) {
		...
		if (conn) {
			...
			wget_http_add_header_line(req, "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n");
...
//				http_add_header_line(req, "If-Modified-Since: Wed, 29 Aug 2012 00:00:00 GMT\r\n");
			...
			if (config.keep_alive)
				wget_http_add_header_line(req, "Connection: keep-alive\r\n");

			if (!config.cache)
				wget_http_add_header_line(req, "Pragma: no-cache\r\n");
			...
			if (config.post_data) {
				...
				wget_http_add_header_line(req, "Content-Type: application/x-www-form-urlencoded");
